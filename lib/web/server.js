...
export class WebUI {
  ...
  /** 配置上是否启用了 WebUI */
  get enabled() {
    return this.cfg?.webui?.enable === true
  }
  ...
  mount(app) {
    ...
    app.use(API_PREFIX, createRateLimiter(), createBodyLimit(), this.createRouter())
   ...
   * `/api/v1` 的路由。
   *
   * `ready` 是前端等待页唯一需要的接口：面板本身要等宿主上线才有数据可显示。
   *
