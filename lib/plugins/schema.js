 ...
 * | `x-widget` | 仅作展示提示（如 `textarea`、`color`），不参与校验 |
 *
 * ## 不支持的关键字
 *
...
/** 支持的 `type` 取值 */
export const SUPPORTED_TYPES = ["object", "array", "string", "number", "integer", "boolean"]
 ...
 * 判断值是否满足给定类型。
 * @param {unknown} value 待判断的值
 * @param {string} type JSON Schema 的类型名
 * @returns {boolean}
 */
function matchesType(value, type) {
  switch (type) {
    ...
    default:
      // 未知类型名：不判定失败，交由上层的 schema 自检去报告
      return true
  }
}
...
export function checkSchema(schema, path = "") {
  ...
  if (type !== undefined && !SUPPORTED_TYPES.includes(String(type))) {
    errors.push({
      path: `${path}.type`,
      message: `不支持的 type「${String(type)}」，可用：${SUPPORTED_TYPES.join(" / ")}`,
    })
  }
...
export function validate(schema, value, path = "") {
  ...
  const type = schema.type === undefined ? undefined : String(schema.type)
  ...
  if (type && !matchesType(value, type)) {
    errors.push({
      path: label,
      message: `期望 ${type}，实际是 ${value === null ? "null" : typeof value}`,
    })
