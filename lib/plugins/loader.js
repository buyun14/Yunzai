...
class PluginsLoader {
  ...
  async deal(e) {
    ...
    Object.defineProperty(e, "isSr", {
      get: () => e.game === "sr",
      // 用块体而不是箭头简写：简写会把赋值结果当返回值，
      // setter 的返回值无任何语义，但 no-setter-return 会报错。
      // 与阶段 2 在 pipeline/stages/process.js 的同一处写法保持一致。
      set: v => {
        e.game = v ? "sr" : "gs"
      },
    })
   ...
   * 处理事件，加入自定义字段
   *
   * 下列字段用描述列表而非 `@param e.msg` 点号名：点号名强制要求父级先声明为
   * `{object}`，而那会把 `e` 从隐式 any **收窄**成 object，函数体里每处
   * `e.xxx` 都变成 TS2339（实测 170 → 198，越修越差）；`{any}` 与
   * `Record<string, any>` 又都不满足 TS8032。三种写法都实测过，
   * 详见 docs/refactor/baseline/static-analysis.md §3.4。
   *
   * - `e.msg` 文本消息，多行会自动拼接
   * - `e.img` 图片消息数组
   * - `e.atBot` 是否at机器人
   * - `e.at` 是否at，多个at 以最后的为准
   * - `e.file` 接受到的文件
   * - `e.isPrivate` 是否私聊
   * - `e.isGroup` 是否群聊
   * - `e.isMaster` 是否管理员
  ...
  reply(e) {
     ...
     * - `data.at` 是否提及用户
     * （第三个参数 `data` 的点号名同样改用描述列表，理由见 dealEvent）
     */
