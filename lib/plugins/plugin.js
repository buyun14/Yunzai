...
export default class plugin {
   ...
   * 会话状态槽的键。
   *
   * 格式是 `<插件名>.<umo>`，其中 `umo` 见 `lib/message/umo.js`。
   * 阶段 5 之前这里是 `<插件名>.<self_id>.<group_id|user_id>`——也就是全仓第四份
   * 手拼的会话键。换成 `umo` 之后，“会话是什么”只剩一处定义。
   *
   * ⚠️ **`isGroup` 与消息自身的群/私聊不是一回事**，别把两者合并：
   * 消息在群里，而插件可能只想为**某个人**开槽（“等这个人回复”）。
   * 所以 `isGroup=false` 用的是 `scope: "user"`（`<adapter>:<self_id>:user:<user_id>`），
   * 而不是私聊会话。群会话只有一个，群里可以有很多个人会话。
   *
   * 键只用作内存 `stateArr` 的下标，不入库；格式变化不会影响已存数据。
   * miao-plugin 自带一份同形的 `conKey` 与自己的 `stateArr`（`components/common/Plugin.js`），
   * 那份不受本处理影响，两份表也从不互相读写。
   *
   * @param {boolean} [isGroup] 是否按群开会话（否则按人）
   * @returns {string} 键
   */
  conKey(isGroup = false) {
    return `${this.name}.${umoOf(
      {
        adapter_id: this.e?.adapter_id,
        adapter_name: this.e?.adapter_name,
        self_id: this.self_id || this.e?.self_id,
        user_id: this.user_id || this.e?.user_id,
        group_id: this.group_id || this.e?.group_id,
      },
