 ...
 * miao-plugin 的可选模块。
 *
 * 它们只在装了该插件（且目录名是 `genshin`）时才存在，因此整块包在 try/catch 里。
 *
 * 路径断言成 `string` 是必需的：写成字符串字面量时 TS 会去解析它并报 TS2307，
 * 而这些模块**本来就不是本仓的依赖**，报错只会污染闸门。断言之后 TS 不再解析，
 * 得到的类型是 any——与运行期 try/catch 的语义一致。
 */
try {
  gsCfg = (await import(/** @type {string} */ ("../../plugins/genshin/model/gsCfg.js"))).default
  MysApi = (await import(/** @type {string} */ ("../../plugins/genshin/model/mys/mysApi.js")))
    .default
  MysInfo = (await import(/** @type {string} */ ("../../plugins/genshin/model/mys/mysInfo.js")))
    .default
  NoteUser = (await import(/** @type {string} */ ("../../plugins/genshin/model/mys/NoteUser.js")))
    .default
  MysUser = (await import(/** @type {string} */ ("../../plugins/genshin/model/mys/MysUser.js")))
    .default
} catch {}
try {
...
export default class Runtime {
    ...
    if (user) {
      e.user = new Proxy(user, {
        get(self, key, receiver) {
          ...
          if (
            ["getUidData", "hasUid", "addRegUid", "delRegUid", "setMainUid"].includes(
              /** @type {string} */ (key),
            )
          ) {
            return (uid, _game = "") => {
   ...
   * @param data 渲染数据
   *
   * 渲染配置的字段用描述列表而非 `@param cfg.retType` 点号名，理由见
