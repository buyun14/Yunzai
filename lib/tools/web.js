 ...
 * 模板调试页的模板引擎。
 *
 * **这个包并没有被安装**（`package.json` 里只有 `art-template`），上游同样如此，
 * 所以 `pnpm web` 现在是跑不起来的。路径断言成 `string` 只是不再让闸门报 TS2307，
 * **不代表它可用**——已登记为缺陷 D4（见 baseline/static-analysis.md）。
 * 去留归阶段 6：那一阶段本来就要重做这个调试页。
 */
const template = /** @type {any} */ (await import(/** @type {string} */ ("express-art-template")))
  .default
...
app.get("/:page", async function (req, res) {
  // 原写法是 `let [plugin, app, ...page] = ...` 紧接 `page = page.join("_")`：
  // 同一个变量先当数组、再当字符串。运行期能跑，但类型上是两回事（也是本次
  // 两处报错的来源）。拆成两个名字，语义完全不变。
  const [plugin, app, ...pageParts] = req.params.page.split("_")
