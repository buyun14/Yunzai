 ...
 * # 来历
 *
 * 这些函数原本是照着 `lib/plugins/loader.js` 翻译的独立实现——阶段 2 做「影子运行对比」时
 * 两侧各跑各的代码，共享实现就只能证明「代码相同」而不是「行为相同」。
 * 阶段 2 第 5 步删掉旧路径后，loader 里的对应方法已经不在了，**这份就是唯一实现**。
 *
 * 每个函数都对应改造前 `deal()` 里的一个方法，语义要求**逐字等价**：
