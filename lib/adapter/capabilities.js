...
import { capabilityOfComponent } from "../message/render.js"
 ...
 * 挑出适配器**确定不支持**的组件。
 *
 * 只挑 `supports()` 返回 `false` 的：`undefined`（未知）不算不支持——这正是本文件
 * 头部那两条约束的落点。调用方拿到清单后应当**降级**（改发文本、或提示一句），
 * 而不是把整条消息丢掉，否则一次错误的声明就会让功能静默消失。
 *
 * @param {unknown} adapter 适配器对象，或适配器的 `id` 字符串
 * @param {import("../message/component.js").Component[]} components 组件数组
 * @returns {import("../message/component.js").Component[]} 确定不支持的组件（可能为空）
 */
export function unsupportedComponents(adapter, components) {
  const result = /** @type {import("../message/component.js").Component[]} */ ([])

  for (const component of components) {
    const capability = capabilityOfComponent(component)
    if (capability && supports(adapter, capability) === false) result.push(component)
  }
