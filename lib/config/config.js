...
class Cfg {
  constructor() {
    ...
    if (this.getAllCfg("bot").file_watch === false) {
      ...
      fs.watch = () => watch
      /**
       * 把 chokidar 的监听换成空实现（`bot.file_watch === false` 时）。
       *
       * 这里的 stub 只实现真正会被调用的那几个方法，因此**刻意**与 chokidar 的
       * `FSWatcher` 类型不兼容。用一次带类型的 `any` 视图把它写清楚，
       * 比在两行上各挂一个 `@ts-expect-error` 更好读，而且同样只影响这两处赋值。
       *
       * （这与"断言一件假事"不同：类型确实对不上，我们是有意替换实现。）
       */
      const chokidarAny = /** @type {any} */ (chokidar)
      chokidarAny.watch = () => watch
      chokidarAny.FSWatcher = FSWatcher
  ...
  /**
   * 丢弃某个配置组的解析缓存，让下次读取重新读盘。
   *
   * `getYaml()` 会把解析结果缓存在 `this.config` 里，而 `watch()` 的失效是
   * **延迟 5 秒**的防抖。所以"面板刚写完配置、马上要生效"这条路径上，
   * 它读到的仍是旧值——除非显式把缓存打掉。
   *
   * 给面板的插件启停用：那条路径是"写盘 → 立刻重算生效范围"，
   * 中间没有 5 秒可以等。
   *
   * @param {string} name 配置组名（`config/config/<name>.yaml` 的文件名）
   * @returns {void} 无
   */
  reload(name) {
    delete this.config[`config.${name}`]
    delete this.config[`default_config.${name}`]
  }
