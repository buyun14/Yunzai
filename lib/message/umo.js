 ...
 * | 某人在群里触发的一次会话（按人而非按群） | `QQ:12345:user:10001` |
 *
 * 前两段是"谁"，后两段是"哪里"。将来若出现频道（guild/channel），
 * 只需扩展 `scope` 的取值，前两段不变。
 *
 * # 四种 scope 的区别（后三种是插件可以选的）
 *
 * `group` / `private` 描述的是**这条消息属于哪个会话**，由事件本身决定；
 * `user` 是给 `Plugin.conKey(false)` 这类需求用的：插件在群里也可能只想为
 * **某个人**开一个槽（“等待这个人回复”）。两者不是一回事，不能混用：
 * 群会话只有一个，而群里可以有很多个人会话。
 *
 * # 为什么是纯函数
 *
 ...
 * 部分第三方适配器可能只给后者。
 *
 * @param {Record<string, unknown>} event 事件对象（或任何带这几个字段的对象）
 * @param {{ scope?: "group" | "user" }} [options] 强制指定作用域。
 *   缺省时按事件自身判定（群 → `group`，其余 → `private`）；
 *   `user` 用于“按人开会话槽”的需求（见上）
 * @returns {string} 形如 `QQ:12345:group:67890` 的会话键
 */
export function umoOf(event, { scope: forceScope } = {}) {
  const kind = event.message_type ?? event.notice_type ?? event.request_type
  const scope = forceScope ?? (kind === "group" ? "group" : "private")
