 ...
 * # 这个文件解决什么问题
 *
 * 现状是「消息形状」全靠隐式约定：`NormalizeStage.parseMessage()` 直接对原始 segment
 * 数组 `switch`（这段逻辑的前身是旧路径的 `PluginsLoader.dealEvent()`，已随阶段 2 第 5 步
 * 一并删除），未识别的类型**静默丢弃**。语音、视频、合并转发、戳一戳这些段到达插件时
 * 完全不可见，插件也没有任何办法知道"收到了一条语音"。
 *
 ...
 * # 为什么组件要保留 `raw`
 *
 * 历史实现里 `e.file` 直接指向**原始 segment 对象**（旧 `dealEvent` 的 `case "file"`），
