...
export default class Yunzai extends EventEmitter {
  ...
  serverAuth(req) {
    ...
    for (const i in cfg.server.auth) {
      if (
        this.authValueMatches(req.headers[i.toLowerCase()], cfg.server.auth[i]) ||
        this.authValueMatches(req.query[i], cfg.server.auth[i])
      )
        continue
   ...
   * 一个鉴权值是否匹配配置里的令牌。
   *
   * 除了**全等**，额外接受标准的 `Bearer <令牌>` 形式。两条实测理由：
   *
   * 1. `Authorization: Bearer <token>` 是 HTTP 约定俗成的写法。SnowLuma
   *    （OneBot v11 客户端）就**自动**给 `Authorization` 值加上 `Bearer ` 前缀，
   *    而配置里的令牌不会带这个前缀——于是它无论用请求头还是查询参数都过不去，
   *    表现为 `Authorization 鉴权失败`，而配置看起来完全正确（实测踩过）。
   * 2. 自己发出去的 URL（`/File/...`）带的是**裸令牌**，所以不能反过来要求
   *    配置里必须写成 `Bearer xxx`——那样自己发的 URL 又认证不了。
   *
   * 只放宽这一个前缀：`Bearer` 之外的内容仍须逐字相等，所以没有削弱鉴权
   * （能构造出这个头的人本来就持有令牌）。
   *
   * @param {unknown} value 请求实际带来的值（头或查询参数）
   * @param {string} token 配置里的令牌
   * @returns {boolean} 是否通过
   */
  authValueMatches(value, token) {
    if (typeof value !== "string") return false
    if (value === token) return true
    return /^Bearer\s+/i.test(value) && value.replace(/^Bearer\s+/i, "") === token
  }
  ...
  wsConnect(req, socket, head) {
    ...
    // 取第一段路径当适配器名。**必须先把查询串切掉**：
    // `serverAuth` 支持用 `?Authorization=<token>` 过鉴权（见上面 `req.query`
    // 与 `serverAuth` 的 `req.query[i] === cfg.server.auth[i]`），而带了查询串的
    // `req.url` 是 `/OneBotv11?Authorization=xxx`——直接 split 会得到
    // `OneBotv11?Authorization=xxx`，`path in this.wsf` 判定失败 → **404**。
    //
    // 后果很隐蔽：鉴权其实**通过了**，但客户端只看到握手失败；而且这恰好把
    // "用查询参数带令牌"这条框架本来就支持的路堵死了，于是 `server.auth` 一填，
    // 所有 OneBot 适配器都连不上（实测 SnowLuma）。
    const path = req.url.split("?")[0].split("/")[1]
    if (!(path in this.wsf)) {
      util.makeLog(
        "error",
  ...
  async run() {
    ...
    this.stat.online = 2
    // 必须在宿主的兜底之前挂载，否则 /api/v1 永远拿不到请求
    WebUI.mount(this.express, { loader: PluginsLoader })
    // 兜底：未命中的请求一律 404 JSON。
    //
    // 刻意**不再**做「跳转到某个站点」：那条旧行为会把任何未命中的路径 302 到
    // 框架作者的仓库地址（`server.redirect`），既对外泄露别人的地址，
    // 也让「这个端口到底是什么服务」变得含糊。想给自己的站点做落地页，
    // 用 `Bot.express.get("/", …)` 显式注册即可（见 BCR-0003）。
    //
    // 用 JSON 与 `/api/v1` 的未命中响应保持同一种形状；这里手工序列化，
    // 不 import lib/web 的 sendJSON——那个模块在构造时要读 WebUI 配置，
    // 而 lib/bot.js 与它是互相 import 的关系，这条链上不值得再引一个模块。
    this.express
      .use((req, res) =>
        res
          .status(404)
      ...
      .use(this.serverError.bind(this))
    util.makeLog(
      "info",
