...
- [ ] 阶段 6：WebUI（**后端已完成**：安全基座、就绪探针、五个接口（含 SSE 日志流）、
      `docs/openapi.yaml` 契约与一致性测试；配置键是 `server.yaml` 的
      `server.webui.enable`（默认 `false`）；三处偏离已登记。
      **只剩 `dashboard/` 前端**，见 `06-webui.md` §5）
- [ ] 阶段 7：AI 能力（按需）
