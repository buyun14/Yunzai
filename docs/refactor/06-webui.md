...
>
> ### v2 第二步已落地：配置写入端点（2026-09-24）
>
> `PUT /api/v1/config/{name}`（`lib/web/api/config-write.js`）。
> **这是整个 WebUI 里第一个会改宿主状态的接口**，登记为 **BCR-0004**。
>
> 四道闸门（任何一道不过都不写盘）：
>
> | # | 闸门 | 不过时 |
...
> | 4 | 新值过该文件的 schema 校验（**不填默认值**） | 400 |
>
> 顺序刻意是"先拒签、后校验"：不给根本不允许写的文件做读盘与构造 Document 的无用功。
> 另外请求体必须带 `confirmed: true`（否则 **428**），写盘是**原子**的
> （同目录临时文件 + `rename`），写前必先备份到 `config/backups/pre-write-<时间戳>-<原名>`
> （目标已存在时加 `.1` `.2` …，**绝不覆盖**）。
>
> **为什么封死 `auth` / `https`**：它们决定面板自己的门卫与监听。
> 能改 `auth` = 一个已通过认证的会话可以换掉令牌或把所有人锁在外面；
> 能改 `https` = 写错证书路径、重启即起不来。与 §3.3「启用 WebUI 必须配 auth」
> 同一条思路：**面板不该有能力削弱自己的门卫**。代价明确——面板不能作为认证配置的
> 管理入口，要改就手动改 yaml（这两处本来也要重启才生效）。
>
> #### 写入过程中抓到并修掉的一个真实缺陷：空键把注释拽走
>
> 第一次真机验证时落盘结果是这样的：
>
> ```yaml
> username: # Redis 密码      ← 注释被拽到了 username 行尾
>
> password: # Redis 数据库
> ```
>
> 根因不在我们的代码：`yaml` 的 loader 对「后面紧跟注释的**空标量**」做了一次前瞻，
> 把那行注释记成那个空标量的**尾注释**。只要值是 `null`，stringify 会把它渲染成
> 下一行的注释、看起来一切正常；**但一旦把这个空键填上值**（正是这个接口最主要的
> 用途），同一条注释就变成行尾注释——归属错了，而且是被我们的写入触发的。
>
> 单独验证过：`parse → toString`（不做任何修改）就会复原出这个形态，
> 所以不是 `doc.set` 的锅，也没有可以关掉它的 parse/stringify 选项。
>
> 修法（`fixEmptyScalarComments()`）：解析后、改值**之前**，把空标量上的
> `value.comment` 挪给下一个键作 `commentBefore`，并清掉解析器留下的 `spaceBefore`
> （它是准备另起一行渲染尾注释用的，注释搬走后会变成空行）。
> 修完 round-trip 逐字节稳定，且填值不再影响任何注释的归属。
> 两条用例盯着它（"填上空键时不会把下一行的注释拽成行尾注释"、
> "不改任何键时注释原样保留"）。
>
> 真机验证（临时启用面板、验完已还原；`redis.yaml` 与 `server.yaml` 都还原成
> 逐字节原样，验证产生的备份已清理）：
>
> ```
> PUT /api/v1/config/redis.yaml  {"config":{"db":3},"confirmed":true}  → 200
>   {"name":"redis.yaml","written":true,"backup":"config\\backups\\pre-write-…-redis.yaml","restartRequired":true}
> 落盘：其他行与全部注释原样，只有 db: 0 → 3
> 备份内容与改动前逐字节一致
> 拒绝路径：
>   改 server.auth（敏感键）   → 400  不允许通过面板修改 auth：它决定面板自身的鉴权与监听…
>   改 server.https（敏感键）  → 400  （同上）
>   改 group.yaml（未建模）    → 400  该配置文件未建模（结构表达不了）…
>   缺 confirmed               → 428  改写配置需要显式确认…
>   值非法（port: 0）          → 400  配置校验失败：port：不能小于 1
>   路径穿越 ../package.json   → 400  文件名不合法
> 拒绝之后 redis.yaml 仍是 db: 3（没有被后续请求改回去）
> ```
>
> **已知缺口（刻意不做）**：写入路径没有单独叠加更紧的限流桶，用的是 `/api/v1`
> 上那个 per-IP 令牌桶；§5 里「敏感接口有 per-IP 限流，压测下返回 429」
> 那条验收项仍未勾选——等重启/更新这类更高频也更危险的接口落地时一起做。
>
> ### v2 第三步已落地：前端表单（2026-09-24）——**v2 完成**
>
> `dashboard/src/components/SchemaForm.vue`（递归组件）+ `views/ConfigView.vue`
> 的查看/编辑双模式。`x-widget` 决定控件：
>
> | `x-widget` | 控件 |
> |---|---|
> | `password` | 默认遮住的文本框 |
> | `textarea` | 多行文本 |
> | `select` | 下拉（配 `enum`） |
> | `raw` | **不渲染**，由页面统一说明"这些键请直接编辑 yaml" |
> | 其余 | 按 `type`：boolean → 开关、integer/number → 数字、string → 文本 |
>
> #### 两处关键设计
>
> **① 提交的是增量，不是整份配置。** 页面在进入编辑时存一份快照，保存时只把
> **值真的变了的键**发出去。必须这样：表单里的值掺了 schema 的 `default`
> （用户文件里其实没有那个键），整体提交等于**用默认值覆盖用户配置**。
> 这是这个界面最容易犯的错。
>
> **② `server.auth` / `server.https` 只能看不能改。** 后端按 BCR-0004 封死了它们，
> 界面必须跟着封——让用户填完再被 400 拒掉是最差的体验。
>
> #### 用 puppeteer 驱动真实浏览器做的验证（这一步暴露了 3 个真 bug）
>
> 「构建通过」证明不了界面能用，所以这一轮用仓库自带的 puppeteer
> （`.puppeteerrc.cjs` 指向本机 Chrome）驱动真实页面，17 项断言全过：
>
> ```
> #/config 打开          → 9 个文件列出
> 点 redis.yaml + 编辑   → 表单渲染出 命令路径/地址/端口/用户名/密码/数据库编号
>                          未改动时保存按钮禁用
> 改 db 0→5             → 显示"改动 1 项 db: 0 → 5"，保存按钮变为可点
> 点保存                → PUT 200，结果里带写前备份路径
>                          **重新读取该文件确认 db=5**（只信界面提示证明不了写盘）
> 控制台               → 0 错误、0 非 2xx
> server.yaml          → 列出 auth/https 及原因（BCR-0004）与"去哪里改"
> ```
>
> 驱动过程中抓到并修掉的三个真问题：
>
> 1. **整个面板一个图标都没有**（我早先引入的回归）。原因是我以为
>    `aliases` 会把名字解析成路径、组件收到的是路径，于是组件又拿"路径"去
>    `iconPaths` 里查一次 → 查不到 → 返回 null → Vuetify 退回 `VClassIcon`
>    渲染 `<i class="mdi-menu">`，浏览器解析 `<path d="mdi-menu">` 失败，
>    控制台刷 `Expected number, "mdi-menu"`。
>    **实测（用 puppeteer 打出真实的 `props.icon`）才知道 Vuetify 3.13 传的是
>    `"mdi-menu"` 这样的原始名字**——名字→路径只能在组件里做。
> 2. **编辑模式一进去就炸**：`structuredClone` 碰到 Vue 的响应式 Proxy 抛
>    `DataCloneError`，表单一个字段都不渲染。改成 JSON 往返拷贝。
> 3. **保存成功的提示看不到**：保存成功后会退出编辑态，而那条提示写在
>    `v-if="editing"` 块里，随之一并消失——而"已写入 + 请重启"恰恰是保存后最该
>    看到的信息。移到编辑块外。
>
> 另有两处**测试自己的错**（值得记一笔，否则会把"测试写错"误判成实现有问题）：
> 断言写的是 schema 的 `title`，而界面上显示的是中文 `title`；
> 以及只在等一条永远不会出现的提示，却没有直接断言写盘结果。
>
> 顺带修掉两处遗留：侧栏那句 `v1 只读面板` 已经不成立（现在是"配置可改，
> 改完要重启"）；`/favicon.ico` 会 401（鉴权覆盖整个站点）并在控制台留一条假错误
> ——`index.html` 里加了一个内联 data URI 图标，不留空就不产生那个请求。
>
> ### v3 第一块：进程控制（重启 / 停止，2026-09-24）
>
> `lib/web/api/control.js` + 状态页顶部的"进程控制"卡片。登记为 **BCR-0005**。
>
> 这两条路径是**整个面板里最危险的**：它们能把服务停掉，而**停止不会自己回来**
> ——面板本身也一起停（它就跑在同一个进程里）。三层防护：
>
> | 层 | 做法 |
> |---|---|
> | 来源闸门 | `createLocalOnlyGate()`：只允许本机与内网，公网一律 **403**（不是 401——401 会让人以为令牌填错了去反复试） |
> | 探测先行 | `GET /api/v1/control` 返回 `canControl`，界面据此把按钮置灰并说明原因，而不是让用户点完才被拒 |
> | 二次确认 | 停止必须弹确认框；重启不需要（它会自己回来） |
>
> ⚠️ **来源判断不看 `X-Forwarded-For`**：那个头客户端可伪造，信它等于给公网一个
> 关机按钮。只有直连地址算数。远程运维请走 VPN / 隧道。有测试专门盯这一条。
>
> #### 重启为什么不复用 `Bot.restart()`
>
> 内核那个按 `global.start_type` 分支，而 `external` 分支在外是 `execve` 之后直接
> `process.exit()`——**Windows 上没有 `execve`，那条路径会让进程直接死掉不回来**。
> 面板改成不依赖启动方式也不依赖 pm2：先关掉自己的 HTTP 监听把端口腾出来，
> 再 `spawn(detached)` 拉起一个命令行完全相同的新进程，然后退出。
>
> **顺序不能反**：`lib/bot.js` 的 `serverEADDRINUSE()` 在端口被占用时会向占用者
> 发一个 `/exit`——新进程先起来的话会把老进程踢掉，变成"重启把自己踢了"。
>
> **先回 202 再动手**：进程一重启连接就断了，先 `sendJSON(202)` 让前端拿到明确的
> "已受理"，再延迟 500ms 执行。否则前端只看到一个网络错误。
>
> 真机验证（真的把进程换了）：
>
> ```
> GET  /api/v1/control          → 200 {"canControl":true,"stopIsRecoverable":false}
> POST /api/v1/control/restart  → 202 {"code":"accepted","action":"restart",…}
>   第 4 秒就绪；PID 6712 → 27644（真的换了一个进程，不是原地假重启）
>   重启后 /dashboard/、/status、/config/schemas、/control 全部 200
> ```
>
...
> 细节记在 `dev-notes.md` §12。
>
> ### v3 第三块：日志暂停/导出 + 首页最近日志 + 配置复核辅助（2026-09-24）
>
> 四处增量，都是"界面更好用"而不是新增能力面（只多了一个**只读**接口，
> 所以不需要 BCR）：
>
> **① 日志暂停**（`LogsView.vue`）
>
> ⚠️ 关键设计：**暂停不断开 SSE 连接**。断开重连只能拿到后端最近 200 条的回放，
> 暂停期间的日志会凭空消失——而"暂停"的用意恰恰是"让我看清眼前这些"或
> "我去查个别的东西"。所以暂停时继续收、攒在 `pending` 里，恢复时整段接上。
> 缓冲同样限长（超过 `MAX_LINES` 丢弃并计数，界面上显示丢了多少），
> 否则"暂停一晚上"会把标签页吃爆。
>
> **② 日志导出**
>
> 导出的是**过滤后的全量**（`shown`，几百行），不是 DOM 里渲染的那些
> （视口限高滚动，DOM 里只有几十个 `.log-line`）。
> 文件名 `yunzai-logs-<本地时间>.txt`；行格式 `ISO时间 [等级] 分类 消息`，
> 等级左对齐补齐，便于在编辑器里成列阅读。
>
> ⚠️ `URL.revokeObjectURL` **不能紧跟 `a.click()`**：同步 revoke 会让下载取不到
> 数据且**不报错**，表现为"点了导出什么都没发生"。推迟 10 秒再释放。
>
> **③ 首页最近日志**
>
> 新增 `GET /api/v1/logs/recent`（**只读**，返回 `{buffered, lines}` 快照）。
> 与 SSE 那条路刻意分开：SSE 一个页面开着就占一个订阅者，而首页只要"最近发生了什么"，
> 为它开长连接是浪费、且切走页面时的清理更容易出错。用轮询（与状态同一个 5 秒周期），
> 失败模式退化成"这块内容不更新"。
>
> **④ 配置页改动标记**
>
> 改动过的顶层键加 `●` 前缀、嵌套块整块变色，并有一个"只看改动"开关
> （大文件里改了两三个键、提交前复核时，逐行扫很费眼）。
>
> 真机验证（puppeteer，17 项断言全过）：
>
> ```
> 首页          → 最近日志卡片存在、9 行、无控制台错误
> 日志暂停      → 提示写明"连接没断"；暂停期间视图行数冻结（15 -> 15）；
>                 恢复后提示消失、视图继续更新
> 日志导出      → 浏览器发起下载 yunzai-logs-20260925-042622.txt（文件名合规）
>                 blob 17175 字符 / 341 行，首行格式正确
> 配置"只看改动" → 改 db 后标签出现 ●；开关打开后字段数 7 -> 2
> ```
>
...
| 项 | 内容 | 前置 |
|---|---|---|
| **v2 配置编辑** | 表单由 schema 渲染；写入前后走同一个同构校验器（阶段 1 的 `lib/plugins/schema.js`） | ✅ **已完成（2026-09-24）**：schema 表（`lib/config/host-schema.js`）、schema 只读端点、写入端点（BCR-0004）、前端表单（`SchemaForm.vue` + `ConfigView.vue` 双模式），并用 puppeteer 在真实浏览器上验过 |
| **v3 插件页面** | 插件自带 `pages/`，受限 iframe + 插件用 `Bot.express` 注册自有 API | 阶段 7（可选） |
| 敏感接口的紧限流 | 重启/更新落地时，在对应子路径上再叠一层更紧的令牌桶 + 二次确认字段 | 待那些接口出现 |
