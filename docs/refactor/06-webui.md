...
> **状态（2026-09-24）：未开工。** 仓库里只有一个空的 `lib/web/api/` 目录（尝试留下的脚手架，
> 未被 git 跟踪）。开工时先读 §3.3——安全项必须与功能同期上线，而
> `config/default_config/server.yaml` 的 `auth` 默认为空，此时 `serverAuth` 完全放行。
> 阶段 5 遗留的「宿主配置 schema 化」（`config/host.schema.js`）可与本阶段合并做。
