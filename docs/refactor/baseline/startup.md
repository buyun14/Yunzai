...
**2026-09-24 真机确认（SnowLuma v1.14.19 + OneBot v11，群 `1083460310`）：预测成立。**

| 时间 | 日志（节选） | 说明 |
|---|---|---|
| 13:48:39 | `群禁言：2135481470 => 1609167206 ban 600秒` | 机器人被禁言 600 秒 |
| 13:48:45 | `[#帮助][喵喵:喵喵帮助(help)][开始处理]` | **照常进入插件处理**——禁言分支没有命中 |
| 13:48:45 | `[图片生成][miao-plugin/help/index][3次] 338.59KB 160ms` | 渲染照常做完，这几百毫秒白花了 |
| 13:48:46 | `[ERRO] 发送消息错误 … send group message rejected: result=120 err=` | 唯一的失败点在**平台侧发送** |
| 13:48:46 | `[完成870]` | 插件调用被正常收尾（`[ERRO]` 不参与该判定） |

作为对照，禁言**之前**同一条 `#帮助`（13:48:04）走完了 `开始处理 → 图片生成 → 发送群消息 → 完成1秒485`，
私聊路径（13:48:09）同样成功——也就是说被测的不是"消息收不到"，而是同一个事件在禁言前后的差别。

两个后果值得记下来：

1. 被禁言时机器人仍会跑完整条处理与渲染（包括昂贵的出图），只在最后一步被平台拒绝，
   **白烧 CPU 与渲染时间**；`AllMuted` 类场景下这是持续的浪费。
2. 「完成」确实打出来了，但它是否同时被 `StatisticsStage` 计为一次**成功处理**，本次**没有验证**——
   若要改，得先看清计数口径，否则修完禁言反而会让统计数字突变。

修法方向（不在本次动手，属于阶段 4 的适配器能力表或阶段 2 第 5 步之后的事）：
把成员禁言状态在 `prepareEvent()` 阶段映射到 `mute_left`，或把判定改为读真实字段
（`group_all_shut` 与成员信息里的 `shut_up_timestamp`）。
