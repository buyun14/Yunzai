...
| BCR-0006 | 6 | `/api/v1` 新增 `PUT /plugins/{name}`：**启用 / 停用插件** | 面板能改 `config/config/group.yaml` 的 `default.disable` 名单，从而改变哪些插件对消息生效——比只改配置更进一步，它直接影响**功能是否可用** | 无额外闸门（与配置写入同级）。写盘走同一套原子写 + 写前备份；请求体必须显式给 `enabled: true\|false` | 默认关闭不受影响。要恢复就再切一次（或直接编辑 `group.yaml`，写前备份里有原内容） | `lib/config/config.js` 新增 `reload(name)` 方法（打掉配置解析缓存，只增不改既有 API） | 已合入（2026-09-24） |
| BCR-0007 | 6 | `/api/v1` 新增 `POST /plugins/{key}/reload`：**不重启重载单个插件** | 面板能替换一个插件的行为而**不必重启宿主**——影响面比重启小得多，但比重载前的状态更"新"，同一份文件在不同时间的行为可能不同 | **不额外限来源**（重启/停止仍限本机与内网）：能改配置的人本来就能改插件文件，重载只是省一次重启。key 做**路径包含检查**（`..`/绝对路径/盘符一律 400），因为 key 会被拼进 `import()` 路径 | 默认关闭不受影响 | 无（复用内核既有的 `changePlugin()` 热更新路径，未改其行为） | 已合入（2026-09-24） |
| BCR-0008 | 6 | **修复**：`wsConnect` 取适配器名时未切掉查询串 | 不是破坏性变更而是修 bug：修复前 `/OneBotv11?Authorization=<token>` 被解析成适配器名 `OneBotv11?Authorization=<token>`，`path in this.wsf` 失败 → **404**。于是 `serverAuth` 本就支持的"用查询参数带令牌"被堵死，**`server.auth` 一填、所有 WS 适配器集体连不上**（实测 SnowLuma） | 无（修复使其按文档工作）。行为变化仅限"以前会 404 的请求现在能通" | 不适用（修 bug） | `lib/bot.js` 的 `wsConnect`：`req.url.split("/")[1]` → `req.url.split("?")[0].split("/")[1]`。对不带查询串的请求行为完全不变 | 已合入（2026-09-24） |
| BCR-0009 | 6 | **修复**：`serverAuth` 的令牌比对只做全等，导致带 `Bearer ` 前缀的客户端一律 401 | 不是破坏性变更而是修 bug：**SnowLuma 会自动给 `Authorization` 值加 `Bearer ` 前缀**，而配置里的令牌不带前缀，于是它用请求头与查询参数两条路都过不去，日志只有一句"鉴权失败"（看起来像令牌填错）。实测 `authorization: 'Bearer c245…'` | 无（**放宽**鉴权值的接受范围）。行为变化仅限"以前会 401 的请求现在能通"，且仍要求令牌内容逐字相等 | 不适用（修 bug） | 新增 `Bot.authValueMatches(value, token)`：全等之外额外接受 `Bearer <令牌>`（scheme 大小写不敏感、允许多空格），`Bearer` 之后仍须逐字相等。**不需要**改配置、也不需要改 SnowLuma | 已合入（2026-09-24） |
