...
第 4 步切朱时留了一个回退开关：`config/config/bot.yaml` 的 `legacy_pipeline: true` 会回退到旧 `deal()`，
缺省走新流水线。开关是**每次提交时**读的，因此支持热更新；状态变化时打一条日志
（`使用新版消息流水线` / `已启用旧版消息流水线`）——回退开关最糟的失败方式是
"用户以为开了其实没开"。

**第 5 步已把开关彻底删除**：`EventBus` 里不再有任何模式判断，`commit()` 永远路由到流水线。
用户配置里残留的 `bot.legacy_pipeline` 现在是**无效果的多余键**（`node . config:diff` 会把它报成多余，
这是正确的）。现在的可观测性换成了一条装配日志：每个档案首次建好阶段实例时输出
`已装配消息流水线（档案 <self_id>，N 个阶段）`。

### 6.3 回退手段

开关没了，回退靠**版本回滚**：流水线阶段是独立模块，
旧实现（`git revert` 到切换前的提交）随时可取。这也是当初只留一个开关而不留两条实现的原因——
两套实现的维护成本会在浸泡期以外持续付出。
...
| 5 | 清理旧路径 | ✅ 已完成（2026-09-24）：`deal()` / `dealEvent()` / `checkLimit()` / `setLimit()` / `onlyReplyAt()` 与 `bot.legacy_pipeline` 开关均已删除；`EventBus` 只剩一条路径；旧侧用例换成基线快照 |
...
> 第 5 步删掉旧路径后，上述"两侧比对"已退役：场景表与观测快照都保留，
> 但只剩流水线一侧，比对对象换成冻结的基线快照（见 `tests/fixtures/pipeline/README.md`）。
> 录制基线时录的是**旧侧**的结果——那是这份场景表最后一次能拿到"经两侧互证的行为"。
...
| 把机器人**禁言**后发 `#状态` | 禁言分支（应为死代码，见下注） + `PreProcessStage` 的 **真实发送失败** 兵底 | ✅ 禁言中照样 `[开始处理]`；发送被 QQ 拒绝（`result=120`）→ 错误走到 `pre-process.js:82` 的 catch 并被记下；`[完成406]` 照常打出，流水线未中断 |
...
- **禁言分支**——已实测确认它在 OneBot v11 路径上**不可达**（不是没测）：
  `mute_left` / `all_muted` 从未被任何适配器写入，禁言中机器人照常响应。
  详见 `baseline/startup.md` O4，归口阶段 4 用适配器能力表 + 规范化字段解决；
...
- [x] 旧实现删除后行为不变：场景表的观测快照与冻结基线逐项一致
      （`tests/fixtures/pipeline/baseline-snapshots.json`，录制自经两侧互证的旧侧结果）
- [x] 真机端到端：`#帮助`（含 miao-plugin 渲染 + 回显）与 `#状态` 在新旧两条路径下均正常
- [x] 真机群聊：群聊路径、`at`/`atBot`、`onlyReplyAt` 不唤醒与别名剥离、群冷却、权限**通过**与**拒绝**、context hook（`conKey` 读真实 `this.e`）、无匹配、**真实发送失败兜底**，全部通过
- [ ] 真机覆盖多账号并发、同文去重的亚秒窗口（见 §7.6）
- [x] `lib/plugins/loader.js` 中 `deal()` 的过程式策略代码已被删除，文件仅剩加载与调度职责（第 5 步）
...
| 旧插件直接调用 `PluginsLoader.deal()` | 低 | 中 | **已删除，且评估为不可达**：全仓与已装插件均无调用点（插件拿到的是事件而不是 loader 实例）；若将来真出现，加一个转发到 `EventBus.commit()` 的薄封装即可 |
