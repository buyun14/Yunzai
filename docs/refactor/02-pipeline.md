...
| `#复读`（**撒回**临时主人后） | `permission: "master"` 的**拒绝**路径：`checkPermission` → `replyOf(event)(message)` | ✅ 回了纯文本提示 `暂无权限，只有主人才能操作`，且**无 `[完成N]` 日志**（拒绝分支在 `runHandler` 之前返回，与旧实现一致） |
...
权限**通过**与**拒绝**两条路径、context hook、无匹配路径、redis 计数维度、`Runtime.init` 实际执行。

仍未覆盖：

- `botAlias` 别名剥离 + `onlyReplyAt: 1` 的"不唤醒"分支（需要改群配置）；
- 禁言 / 群冷却 / 同文去重（需要人为构造时序或让机器人被禁言）；
...
- [x] 真机群聊：群聊路径、`at`/`atBot`、权限**通过**与**拒绝**、context hook（`conKey` 读真实 `this.e`）、无匹配，全部通过且零 error
- [ ] 真机覆盖 `botAlias`/`onlyReplyAt` 不唤醒分支、禁言/限流、多账号并发（见 §7.6）
